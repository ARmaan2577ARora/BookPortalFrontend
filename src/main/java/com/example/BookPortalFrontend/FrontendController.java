...
public class FrontendController {
    ...
    public String books(
        ...
        Map<String, Object> response = backend.get(
                "/internal/catalog/books",
                Map.of(
                        "q", q == null ? "" : q,
        ...
        model.addAttribute(
                "filterText",
                authorId != null ? "Books by selected author"
                        : publisherId != null ? "Books by selected publisher"
                        : ""
        );
    ...
    public String compareStores(
            @PathVariable(name = "bookId") Integer bookId,
            @RequestParam(name = "city", required = false) String city,
            Model model) {

        Map<String, Object> response = backend.get(
                "/internal/catalog/books/" + bookId + "/stores",
    ...
    public String addCart(
            @RequestParam(name = "storeBookId") Integer storeBookId,
            @RequestParam(name = "quantity", defaultValue = "1") Integer quantity,
            @RequestParam(name = "redirectTo", defaultValue = "/cart") String redirectTo,
            HttpSession session,
    ...
    public String updateCart(
        ...
        cart.update(session, storeBookId, quantity);
    ...
    public String removeCart(
        ...
        cart.remove(session, storeBookId);
        return "redirect:/cart";
    }
    ...
    public String checkout(Model model, HttpSession session, RedirectAttributes ra) {
        if (!currentUser.loggedIn(session)) {
            ra.addFlashAttribute("error", "Please login before checkout");
            return "redirect:/login";
        }

        if (cart.isEmpty(session)) {
        ...
        model.addAttribute(
                "addresses",
                backend.list(backend.get("/internal/users/" + email + "/addresses"), "addresses")
        );
    ...
    public String addAddress(
            @RequestParam(name = "houseAddress") String houseAddress,
            @RequestParam(name = "city") String city,
            @RequestParam(name = "state", required = false) String state,
            @RequestParam(name = "zipcode", required = false) String zipcode,
            @RequestParam(name = "country", defaultValue = "India") String country,
            @RequestParam(name = "makeDefault", defaultValue = "false") boolean makeDefault,
            HttpSession session,
            RedirectAttributes ra) {
        ...
        try {
            backend.post(
                    "/internal/users/" + currentUser.email(session) + "/addresses",
                    Map.of(
    ...
    public String placeOrder(
        ...
        try {
            Map<String, Object> response = backend.post(
                    "/internal/checkout/place-order",
                    Map.of(
                            "email", currentUser.email(session),
                            "addressId", addressId,
                            "paymentMethod", paymentMethod,
                            "items", cart.checkoutItems(session)
                    )
            );
    ...
    public String orderConfirmation(
        ...
        List<Map<String, Object>> orders = backend.list(
                backend.get("/internal/checkout/orders/" + currentUser.email(session)),
                "orders"
        );

        Map<String, Object> order = orders.stream()
                .filter(o -> String.valueOf(orderId).equals(String.valueOf(o.get("orderId"))))
                .findFirst()
                .orElse(Map.of("orderId", orderId));
    ...
    public String myOrders(Model model, HttpSession session) {
        ...
        model.addAttribute(
                "orders",
                backend.list(backend.get("/internal/checkout/orders/" + currentUser.email(session)), "orders")
        );
    ...
    public String cancelOrder(
        ...
        try {
            backend.post(
                    "/internal/checkout/orders/" + currentUser.email(session) + "/" + orderId + "/cancel",
                    Map.of()
            );
    ...
    public String authorsPublishers(
            @RequestParam(name = "q", required = false) String q,
            Model model) {

        Map<String, Object> response = backend.get(
                "/internal/catalog/authors-publishers",
                Map.of("q", q == null ? "" : q)
        );
    ...
    public String authorDetail(
        ...
        Map<String, Object> booksResponse = backend.get(
                "/internal/catalog/books",
    ...
    public String publisherDetail(
            @PathVariable(name = "publisherId") Integer publisherId,
            Model model) {
        ...
        Map<String, Object> booksResponse = backend.get(
                "/internal/catalog/books",
                Map.of("publisherId", publisherId)
        );
    ...
    public String profile(Model model, HttpSession session) {
        ...
        model.addAttribute(
                "addresses",
                backend.list(backend.get("/internal/users/" + currentUser.email(session) + "/addresses"), "addresses")
        );
    ...
    public String updateStatus(
        ...
        try {
            backend.post(
                    "/internal/admin/orders/" + orderId + "/status",
    ...
    public String oauth2Callback(
        ...
        try {
            Map<String, Object> user = backend.map(
                    backend.get("/internal/oauth2/session", Map.of("token", token)),
                    "user"
            );
    ...
    public String doSignup(
        ...
        try {
            ...
            ra.addFlashAttribute("success", "Signup successful. Please login.");
            return "redirect:/login";
